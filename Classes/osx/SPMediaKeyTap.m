...
appSwitched(EventHandlerCallRef nextHandler, EventRef evt, void *userData);
...
{
	static SPMediaKeyTap	*_sharedMediaKeyTap = nil;
	static dispatch_once_t	onceToken;

	dispatch_once(&onceToken,
		^{_sharedMediaKeyTap = [[SPMediaKeyTap alloc] init]; });

	return _sharedMediaKeyTap;
}
...
- (void)startWatchingAppSwitching;
{
	...
	OSStatus		err			=
		InstallApplicationEventHandler(NewEventHandlerUPP(appSwitched), 1,
			&eventType, self, &_app_switching_ref);
	assert(err == noErr);

	eventType.eventKind = kEventAppTerminated;
	err = InstallApplicationEventHandler(NewEventHandlerUPP(appTerminated), 1,
			&eventType, self, &_app_terminating_ref);
	assert(err == noErr);
}
- (void)stopWatchingAppSwitching;
{
...
- (void)startWatchingMediaKeys;
{
	...
	_eventPortSource =
		CFMachPortCreateRunLoopSource(kCFAllocatorSystemDefault, _eventPort, 0);
	assert(_eventPortSource != NULL);

	// Let's do this in a separate thread so that a slow app doesn't lag the event
	// tap
...
{
#ifdef _DEBUG
		// breaking in gdb with a key tap inserted sometimes locks up all mouse and
		// keyboard input forever, forcing reboot
		return NO;

#else
		// XXX(nevyn): MediaKey event tap doesn't work on 10.4, feel free to figure
		// out why if you have the energy.
		return ![[NSUserDefaults standardUserDefaults]
			   boolForKey:kIgnoreMediaKeysDefaultsKey] &&
...
+ (NSArray *)defaultMediaKeyUserBundleIdentifiers;
{
	return [NSArray
		   ...
		   @"com.apple.iWork.Keynote", @"com.apple.iPhoto", @"org.videolan.vlc",
		   ...
		   @"fm.last.Last.fm", @"fm.last.Scrobbler", @"com.beatport.BeatportPro",
		   @"com.Timenut.SongKey",
		   @"com.macromedia.fireworks",	// the tap messes up their mouse input
		   @"at.justp.Theremin", @"ru.ya.themblsha.YandexMusic",
		   @"com.jriver.MediaCenter18", @"com.jriver.MediaCenter19",
		   @"com.jriver.MediaCenter20", @"co.rackit.mate", @"com.ttitt.b-music",
		   @"com.beardedspice.BeardedSpice", nil];
}
...
- (void)setShouldInterceptMediaKeyEvents:(BOOL)newSetting;
{
	...
	if (_tapThreadRL && (oldSetting != newSetting)) {
		...
		NSTimer *timer = [NSTimer timerWithTimeInterval:0
			invocation	:[grab invocation]
			repeats		:NO];
		CFRunLoopAddTimer(_tapThreadRL, (CFRunLoopTimerRef)timer,
	...
	CGEventRef event, void *refcon)
{
	...
	if ((type != NX_SYSDEFINED) ||
		([nsEvent subtype] != SPSystemDefinedEventMediaKeys)) {
		return event;
	}
	...
	[self performSelectorOnMainThread:@selector(handleAndReleaseMediaKeyEvent:)
	withObject		:nsEvent
	waitUntilDone	:NO];
...
NSString *kMediaKeyUsingBundleIdentifiersDefaultsKey =
...
- (void)mediaKeyAppListChanged;
{
	 ...
	 *   ) autorelease];
	 *   NSString *bundleIdentifier = [processInfo
	 *   objectForKey:(id)kCFBundleIdentifierKey];
...
- (void)appIsNowFrontmost:(ProcessSerialNumber)psn;
{
	...
	NSDictionary *processInfo = [(id)ProcessInformationCopyDictionary(
			&psn, kProcessDictionaryIncludeAllInformationMask) autorelease];
	...
	EventRef evt, void *userData)
{
	...
	GetEventParameter(evt, kEventParamProcessID, typeProcessSerialNumber, NULL,
		sizeof(deadPSN), NULL, &deadPSN);
